...
One issue is that the `internal structure of ``Expr`` objects`__ doesn't match
their constructor exactly, so one has to put arguments in brackets, as well
as capture the ``typ`` field of macros.

__ http://docs.julialang.org/en/release-0.4/manual/metaprogramming/#program-representation
